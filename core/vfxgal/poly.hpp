	...
	typename Vec::BaseType& c1, typename Vec::BaseType& c2, typename Vec::BaseType& c3)
{
	...
	T area = fabs(length_calc::get(p12.cross(p13))); // missing *0.5 intentional
	...
	// c2
	vec_type p1p = p-p1;
	T subarea = fabs(length_calc::get(p1p.cross(p13)));
	c2 = subarea/area;

	// c3
	subarea = fabs(length_calc::get(p12.cross(p1p)));
	c3 = subarea/area;
