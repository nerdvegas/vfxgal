...
namespace dgal {
	 ...
	 * @param[out] cells Resulting cell meshes.
	...
	const _tbb::blocked_range<unsigned int>& r) const
{
	...
	for(unsigned int ri=r.begin(); ri!=r.end(); ++ri)
	{
		...
		{
			// the cell intersects the mesh surface - perform clipping. Sort
			...
			if(vmesh.m_type == INTERSECT_INSIDE)
			{
				...
				//convert_to_simple(*m_mesh, vmesh.m_mesh);
	...
	const VoronoiSettings& s)
{
	...
	{
		//drd::Timer tm2("qvoronoi");
		detail::qhull_createVoronoiCells(cell_sites, vpoints, vcells);
	}

	//drd::Timer tm1("voronoiFractureMesh3D");
