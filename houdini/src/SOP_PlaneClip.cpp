...
#include <vfxgal/algorithm/clipMesh3D.hpp>
...
OP_ERROR SOP_PlaneClip::cookMySop(OP_Context &context)
{
	...
	vfxgal_hou::ScopedCook scc(*this, context, "Performing drd plane clip");
	if(!scc.good())
		return error();
	...
	add_simple_mesh(*gdp, cmesh, polyIDAttrib.toStdString(), &polyIDs);
